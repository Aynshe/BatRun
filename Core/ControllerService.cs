...
namespace BatRun.Core
{
    public class ControllerService : IDisposable
    {
        ...
        // EN: Emergency stop event — fired when Select+Start is held for 5 seconds
        // FR: Événement d'arrêt d'urgence — déclenché quand Select+Start est maintenu 5 secondes
        public event EventHandler? EmergencyStopRequested;

        // EN: State tracking for Select+Start long press (5 seconds)
        // FR: Suivi d'état pour la pression longue Select+Start (5 secondes)
        private bool _isEmergencyComboHeld = false;
        private DateTime _emergencyHoldStart = DateTime.MinValue;
        private const int EMERGENCY_HOLD_SECONDS = 5;
                            ...
                            try
                            {
                                ...
                                if (controllerConfig?.Mappings != null)
                                {
                                        ...
                                        controllerConfig.Mappings.TryGetValue("StartButton", out string? startValue))
                                    {
                                        ...
                                        {
                                            // EN: Emergency stop long-press detection (Select+Start held 5 seconds)
                                            // FR: Détection de la pression longue Select+Start (5 secondes) pour l'arrêt d'urgence
                                            if (!_isEmergencyComboHeld)
                                            {
                                                ...
                                                _isEmergencyComboHeld = true;
                                                _emergencyHoldStart = DateTime.UtcNow;
                                                _emergencyTriggered = false;
                                                _logger.LogInfo($"[EmergencyStop] Select+Start hold STARTED on {joystickName}");
                                            }
                                            else if (!_emergencyTriggered && (DateTime.UtcNow - _emergencyHoldStart).TotalSeconds >= EMERGENCY_HOLD_SECONDS)
                                            {
                                                // EN: Held for 5 seconds — fire emergency stop event
                                                // FR: Maintenu 5 secondes — déclencher l'arrêt d'urgence
                                                _emergencyTriggered = true;
                                
                                                if (IsInputBlocked)
                                                {
                                                    // EN: [BUG R FIX] Input is blocked (e.g. during game launch phase) — suppress
                                                    // the emergency stop to prevent spurious virtual controller inputs from
                                                    // triggering EmergencyStop while the Moonlight stream is initializing.
                                                    // FR: [BUG R FIX] L'input est bloqué (ex: pendant la phase de lancement) —
                                                    // supprimer l'arrêt d'urgence pour éviter que les inputs parasites de la
                                                    // manette virtuelle ne déclenchent EmergencyStop pendant l'initialisation du stream.
                                                    _logger.LogInfo($"[EmergencyStop] Select+Start held {EMERGENCY_HOLD_SECONDS}s on {joystickName} — SUPPRESSED (IsInputBlocked=true, likely game launching)");
                                                }
                                                else
                                                {
                                                    _logger.LogInfo($"[EmergencyStop] Select+Start held {EMERGENCY_HOLD_SECONDS}s on {joystickName} — EMERGENCY STOP REQUESTED");
                                            ...
                                            // EN: Normal short-press hotkey (only if not in emergency hold or already triggered)
                                            // FR: Hotkey pression courte normale (seulement si pas en maintien urgence ou déjà déclenché)
                                            if (!_isEmergencyComboHeld || (DateTime.UtcNow - _emergencyHoldStart).TotalSeconds < EMERGENCY_HOLD_SECONDS)
                                            {
                                        ...
                                        {
                                            // EN: At least one button released — reset the emergency hold timer
                                            // FR: Au moins un bouton relâché — réinitialiser le timer de maintien urgence
                                            if (_isEmergencyComboHeld)
                                            {
                                                double heldSeconds = (DateTime.UtcNow - _emergencyHoldStart).TotalSeconds;
                                                _logger.LogInfo($"[EmergencyStop] Select+Start released after {heldSeconds:F1}s on {joystickName} — timer RESET");
                                                _isEmergencyComboHeld = false;
                                                _emergencyTriggered = false;
                                                _emergencyHoldStart = DateTime.MinValue;
                                            }
                                        }
                                    }
                                }
