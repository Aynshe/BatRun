            ...
            try
            {
                ...
                var videoThread = new Thread(() =>
                {
                    try
                    {
                        ...
                        // Always track time changes to keep the watchdog happy
                        // EN: Update lastTimeChanged on every tick, regardless of muteAfterFirst option
                        // FR: Mise à jour à chaque tick pour éviter le déclenchement abusif du watchdog
                        mediaPlayer.TimeChanged += (s, e) => { lastTimeChanged = Environment.TickCount; };
