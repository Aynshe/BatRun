...
namespace BatRun.Core
{
    public class ShellCommandExecutor
    {
        ...
        public async Task ExecuteShellCommandsAsync(bool isRestart = false)
        {
            try
            {
                        ...
                        switch (item.Type)
                        {
                            ...
                            case CommandType.Application:
                                if (item.OnlyOneInstance && IsApplicationAlreadyRunning(item.Path))
                                {
                                    ...
                                    if (item.AutoHide)
                                    {
                                        string exeName = Path.GetFileNameWithoutExtension(item.Path);
                                        var procs = Process.GetProcessesByName(exeName);
                                        foreach (var p in procs) await HandleAutoHide(p, item.Path);
                                    }
                                    continue;
                                }
                                ...
                                if (isRestart)
                                {
                                    logger.LogInfo($"BatRun restarted by Guardian. Checking if {item.Path} is already running to avoid duplicate.");
                                    if (IsApplicationAlreadyRunning(item.Path))
                                    {
                                        logger.LogInfo($"Application {item.Path} is already running. Skipping launch due to restart.");
                                        if (item.AutoHide)
                                        {
                                            string exeName = Path.GetFileNameWithoutExtension(item.Path);
                                            var procs = Process.GetProcessesByName(exeName);
