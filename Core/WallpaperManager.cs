...
namespace BatRun.Core
{
    public class WallpaperManager
    {
        ...
        // EN: Completely hide the wallpaper window so it doesn't cover other apps (e.g. EmulationStation in Arcade mode)
        // FR: Cache complètement le wallpaper pour qu'il ne couvre pas les autres applis (ex: ES en mode Arcade)
        public void HideWallpaperWindow()
        {
            if (wallpaperForm != null && !wallpaperForm.IsDisposed)
            {
                if (wallpaperForm.InvokeRequired)
                    wallpaperForm.Invoke(new Action(() => wallpaperForm.Hide()));
                else
                    wallpaperForm.Hide();
            }
        }

        // EN: Restore the wallpaper window visibility
        // FR: Rend le wallpaper à nouveau visible
        public void ShowWallpaperWindow()
        {
            if (wallpaperForm != null && !wallpaperForm.IsDisposed)
            {
                if (wallpaperForm.InvokeRequired)
                    wallpaperForm.Invoke(new Action(() => wallpaperForm.Show()));
                else
                    wallpaperForm.Show();
            }
        }
