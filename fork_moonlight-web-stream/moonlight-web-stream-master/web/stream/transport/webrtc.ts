...
export class WebRTCTransport implements Transport {
    ...
    private onConnectionStateChange() {
        ...
        if (this.peer.connectionState == "connected") {
            ...
            // EN: Signal to parent window that connection is established
            // FR: Signaler à la fenêtre parente que la connexion est établie
            try {
                window.parent.postMessage({ type: 'ML_CONNECTED' }, '*');
            } catch (e) {
                this.logger?.debug("Failed to send ML_CONNECTED postMessage");
            }
    ...
    private initChannels() {
        ...
        for (const channelRaw in TransportChannelId) {
            ...
            this.channels[id] = new WebRTCDataTransportChannel(channel as any, null)
        }
    }
