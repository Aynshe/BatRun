...
class MainApp implements Component {
    ...
    private async refreshUserRole() {
        ...
        // [BATRUN-FORK]: Block dashboard access for service account
        if (this.user.name === "batrun_service" && !new URLSearchParams(location.search).get('arcadeMode')) {
            document.body.innerHTML = `
                <div style="height:100vh; display:flex; flex-direction:column; align-items:center; justify-content:center; background:#050510; color:white; font-family:sans-serif; text-align:center; padding:20px;">
                    <h1 style="color:#00f2ff;">BatRun Arcade</h1>
                    <p>Dashboard access is restricted in Arcade Mode.</p>
                </div>
            `;
            return;
        }
