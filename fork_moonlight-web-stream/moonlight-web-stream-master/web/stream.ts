...
async function startApp() {
    ...
    const arcadeMode = queryParams.get("arcadeMode") === "1"
    ...
    // Start and Mount App
    const app = new ViewerApp(api, hostId, appId, bootstrapRole.role, arcadeMode)
    app.mount(rootElement);

    // [BATRUN-v3]: Wild Exit detection inside the stream page — notify server to cleanup session
    // FR: Détection de sortie brutale dans la page de stream — notifier le serveur de nettoyer la session
    window.addEventListener("beforeunload", () => {
        const token = localStorage.getItem('batrun_token') || '';
        // EN: Always use absolute path to ensure it reaches the Hub even when proxied
        const url = '/api/host/cancel?token=' + encodeURIComponent(token);
...
class ViewerApp implements Component {
    ...
    // [BATRUN-FORK]: Lobby input blocking flag — when true, gamepad state is not sent to host
    // FR: Drapeau de blocage des inputs du lobby — si true, l'état manette n'est pas envoyé à l'hôte
    private _batrunLobbyInputBlocked: boolean = false
    // [BATRUN-v3]: Select+Start 5s hold detection inside the iframe
    // FR: Détection de la pression longue Select+Start 5s dans l'iframe
    private _batrunEmergencyComboActive: boolean = false
    private _batrunEmergencyHoldStart: number = 0
    private _batrunEmergencyTriggered: boolean = false
    private static readonly BATRUN_EMERGENCY_HOLD_MS = 5000
    private static readonly BATRUN_EMERGENCY_OVERLAY_DELAY_MS = 800

    constructor(api: Api, hostId: number, appId: number, bootstrapRole: DetailedRole, arcadeMode: boolean = false) {
        this.api = api
        ...
        setSidebar(this.arcadeMode ? null : this.sidebar)
        ...
        // [BATRUN-FORK]: Listen for lobby input block/unblock messages from parent page
        // [BATRUN-FORK-v8]: Also listen for BATRUN_STOP_STREAM to cleanly stop the stream
        // before starting a new one, so the virtual controller disconnects on the host.
        // FR: Écouter les messages de blocage/déblocage des inputs du lobby depuis la page parente.
        // FR: Écoute aussi BATRUN_STOP_STREAM pour arrêter proprement le stream avant d'en
        // démarrer un nouveau, afin que la manette virtuelle se déconnecte sur l'hôte.
        window.addEventListener("message", (event: MessageEvent) => {
        if (event.data && event.data.type === "BATRUN_LOBBY_INPUT_BLOCK") {
        this._batrunLobbyInputBlocked = !!event.data.blocked
        console.log(`[BATRUN] Lobby input blocked: ${this._batrunLobbyInputBlocked}`)
        }
        if (event.data && event.data.type === "BATRUN_STOP_STREAM") {
        console.log(`[BATRUN] Stop stream requested via postMessage`)
        const stream = this.stream
        if (stream) {
        stream.stop().then((success: boolean) => {
        console.log(`[BATRUN] Stream stopped cleanly: ${success}`)
        }).catch(() => {})
        }
        }
        })
    ...
    private async startStream(hostId: number, appId: number, permissions: StreamPermissions, settings: Settings, browserSize: [number, number]) {
        ...
        // [BATRUN]: Auto-enable FPS counter if setting is enabled
    ...
    onGamepadUpdate() {
        // [BATRUN-FORK]: If lobby input is blocked, send neutral/empty gamepad state instead
        // FR: Si les inputs du lobby sont bloqués, envoyer un état manette neutre/vide à la place
        if (this._batrunLobbyInputBlocked) {
            const input = this.stream?.getInput()
            if (input) {
                // Send empty state for all connected gamepads to prevent ES from reacting
                const emptyState = {
                    buttonFlags: 0,
                    leftTrigger: 0,
                    rightTrigger: 0,
                ...
                for (let i = 0; i < 4; i++) {
                    input.sendController(i, emptyState)
                }
            }
        } else {
        ...
        // [BATRUN-v3]: Detect Select+Start 5s hold inside the iframe
        // FR: Détecter la pression longue Select+Start 5s dans l'iframe
        // This allows P2 (or any player in the Moonlight iframe) to trigger force-stop
        // even when the parent page cannot access gamepads due to iframe focus.
        if (this.arcadeMode && !this._batrunLobbyInputBlocked && !this._batrunEmergencyTriggered) {
            let anyComboActive = false
            const gps = navigator.getGamepads()
            for (let i = 0; i < gps.length; i++) {
                const gp = gps[i]
                if (!gp) continue
                // Select = button 8 (Back), Start = button 9
                ...
                if (isSelectDown && isStartDown) {
                    anyComboActive = true
                    if (!this._batrunEmergencyComboActive) {
                        this._batrunEmergencyComboActive = true
                        this._batrunEmergencyHoldStart = Date.now()
                        console.log('[BATRUN-iframe] Select+Start hold STARTED')
                    }
                    const elapsed = Date.now() - this._batrunEmergencyHoldStart
                    if (elapsed >= ViewerApp.BATRUN_EMERGENCY_HOLD_MS) {
                        console.log('[BATRUN-iframe] Select+Start 5s hold reached! Sending BATRUN_FORCE_STOP to parent...')
                        this._batrunEmergencyTriggered = true
                        this._batrunEmergencyComboActive = false
                        // Notify parent page to show force-stop confirmation
                        window.parent.postMessage({ type: 'BATRUN_FORCE_STOP' }, '*')
                    }
                    break
                }
            }
            if (!anyComboActive && this._batrunEmergencyComboActive) {
                const elapsed = Date.now() - this._batrunEmergencyHoldStart
                this._batrunEmergencyComboActive = false
                this._batrunEmergencyHoldStart = 0
                if (elapsed > 0) {
                    console.log(`[BATRUN-iframe] Select+Start released after ${(elapsed/1000).toFixed(1)}s — timer RESET`)
                }
            }
        }
        // Reset trigger flag when combo is no longer held (allow re-trigger after release)
        if (this._batrunEmergencyTriggered) {
            let anyComboActive = false
            const gps = navigator.getGamepads()
            for (let i = 0; i < gps.length; i++) {
                const gp = gps[i]
            ...
            if (!anyComboActive) {
                this._batrunEmergencyTriggered = false
            }
        }
    ...
    private checkFullyImmersed() {
        // [BATRUN-FORK]: In arcade mode, always keep sidebar hidden
        if (this.arcadeMode) {
            setSidebar(null)
            return
        }
