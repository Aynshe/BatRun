...
function deepMerge(target: any, source: any) {
    ...
    for (const key in source) {
        ...
        if (
            ...
            // EN: Ensure we don't lose existing target properties when merging nested objects
            // FR: S'assurer de ne pas perdre les propriétés cibles existantes lors de la fusion d'objets imbriqués
...
export class StreamSettingsComponent implements Component {
    ...
    private showFpsCounter: InputComponent
    ...
    constructor(permissions: StreamPermissions, settings: Settings) {
        ...
        this.bitrate = new InputComponent("bitrate", "number", i.bitrate, {
            ...
            numberSlider: {
                ...
                range_max: this.permissions.maximum_bitrate_kbps ?? 100000,
            }
        })
        ...
        // [BATRUN]: Show FPS Counter
        this.showFpsCounter = new InputComponent("showFpsCounter", "checkbox", i.showFpsCounter, {
            checked: settings?.showFpsCounter ?? defaultSettings_.showFpsCounter
        })
        this.showFpsCounter.addChangeListener(this.onSettingsChange.bind(this))
        this.showFpsCounter.mount(this.divElement)
    ...
    getStreamSettings(): Settings {
        ...
        if (!settings.controllerConfig) {
            settings.controllerConfig = { invertAB: false, invertXY: false, sendIntervalOverride: null };
        }
        ...
        settings.showFpsCounter = this.showFpsCounter.isChecked()
